function submit_old(partId, webSubmit)
  ...
  for s = 1:numel(submitParts)
    ...
    if (~webSubmit) % submit directly to server
      ...
      [result, str] = submitSolution(login, ch_resp, thisPartId, ...
             output(thisPartId), source(thisPartId), signature);
...
function [email,ch,signature,auxstring] = getChallenge(email, part)
  fprintf('url:(%s)', challenge_url());
  str = urlread(challenge_url(), 'post', {'email_address', email,...
                      'assignment_part_sid', [homework_id() '-' num2str(part)],...
